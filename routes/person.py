...
def create(person: schemas.PersonCreate, _admin: Person = Depends(require_admin), db: Session = Depends(get_db)):
    return crud.create_person(db, person)

@router.get("/{person_id}", response_model=schemas.PersonOut)
def read(person_id: int, _admin: Person = Depends(require_admin), db: Session = Depends(get_db)):
...
def update(person_id: int, person: schemas.PersonCreate, _admin: Person = Depends(require_admin), db: Session = Depends(get_db)):
...
def delete(person_id: int, _admin: Person = Depends(require_admin), db: Session = Depends(get_db)):
